...
function totalFruit(fruits: number[]): number {
	let fruitBasket = new Map<number, number>();
	let numOfFruit = 0;
	for (let l = 0, r = 0; r < fruits.length; r++) {
		fruitBasket.set(fruits[r], (fruitBasket.get(fruits[r]) || 0) + 1);
		// remove left side of window
		while (fruitBasket.size > 2) {
			fruitBasket.set(fruits[l], fruitBasket.get(fruits[l]) - 1);
			if (fruitBasket.get(fruits[l]) === 0) fruitBasket.delete(fruits[l]);
			l++;
		}
		numOfFruit = Math.max(numOfFruit, r - l + 1);
	}
	return numOfFruit;
}
