...
export class BT {
    ...
    public btDiameter(node: IBT = this): number {
        ...
        let getHeight = (node: IBT) => {
            ...
            console.log(`node.value: ${node.value}, leftHt: ${left}, rightHt: ${right}, diameter: ${diameter}`)
    ...
    public printTree(node: IBT = this, spaceCount = 0, label ='* '){    
        ...
        console.log(
            `${' -'.repeat(spaceCount)}${label}  ${node.value}`
        )
...
let btDiameterTest1: {[key: string]: number | number[]} = {
    nodes: [
        1, 3, 2, 7, 4, null,  null, 8, null, 
        null, 5, null, null, null, null, 9, 
