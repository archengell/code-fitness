...
function levenshteinDistance1(str1, str2) {
    ...
    let edits = Array.from(Array(2), () => new Array(largeStrLen).fill(0));
...
function levenshteinDistance_NA(str1, str2) {
    let map1 = new Map();
    let map2 = new Map();
    ...
    let operations = Math.abs(map1.size - map2.size);
    let largerSubStr = str1.length >= str2.length ? str1 : str2;
    for (let char of largerSubStr) {
        if (map1.has(char) && map1.has(char))
            operations += Math.abs(map1.get(char) - map2.get(char));
...
let levenshteinDist1 = {
    "str1": "abc",
    "str2": "yabd"
};
let levenshteinDist2 = {
    "str1": "abc",
    "str2": "yabd"
};
let levenshteinDist3 = {
    "str1": "abc",
    "str2": "yabd"
};
