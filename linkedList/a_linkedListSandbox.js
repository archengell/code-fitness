...
class LinkedList {
    constructor(arr) {
        ...
        if (utils.isNumberArr(arr)) {
            Array.from(arr, (item) => this.addLast(item[0]));
        }
        else {
            Array.from(arr, (item) => this.addLast(item.id, item.data));
        }
    }
    /**
     * @name findBy
     ...
     */
    addFirst(id, data = null) {
        const newNode = new LinkedListNode(id, data);
     ...
     */
    addAt(id, data = null, position = 0) {
        if (position === 0)
            return this.addFirst(id, data);
        if (position === this.length)
            return this.addLast(id, data);
...
let linkedList = new LinkedList(linkedlistTest);
