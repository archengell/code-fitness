...
class DoublyLinkedList {
    constructor(arr) {
        ...
        arr.map((item) => this.setHead(new LinkedList.DLL_Node(item)));
    }
    // o(1) time + space
    setHead(node) {
        if (this.head === null) {
    ...
    find(node) {
        if (this.length === 0)
            return;
        let nodeElem = this.head;
        while (nodeElem !== null) {
            let item = nodeElem.next;
    ...
    // o(1) time & space
    insertBefore(node = null, nodeToInsert) {
    ...
    // o(1) time & space
    insertAfter(node = null, nodeToInsert) {
        if (nodeToInsert === this.head && nodeToInsert === this.tail)
            return;
        this.remove(nodeToInsert);
