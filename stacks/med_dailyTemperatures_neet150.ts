...
//time => o(n) space => o(n)
function dailyTemperaturesBest(temps: number[]): number[] {
	const stack = [];
	const res: number[] = new Array(temps.length).fill(0);

	for (let t = 0; t < temps.length; t++) {
		while (stack.length > 0 && temps[stack[stack.length - 1]] < temps[t]) {
			const idx = stack.pop();
			res[idx] = t - idx;
		}
		stack.push(t);
...
function dailyTemperatures_brute_On2(temperatures: number[]): number[] {
	...
	for (let today = 0; today < temperatures.length; today++) {
		for (
			let forecastedWarmerDay = today + 1;
			forecastedWarmerDay < temperatures.length;
			forecastedWarmerDay++
		) {
