...
function sunsetViews(buildings: number[], direction: string) {
	...
	while (idx >= 0 && idx < buildings.length) {
		...
		while (bldgStack.length > 0 && buildings[bldgStack[bldgStack.length - 1]] <= bldgHt) {
			bldgStack.pop();
		}
		bldgStack.push(idx);
		idx += step;
...
sunsetViews(
	sunsetViewsTests['test1']['buildings'] as number[],
	sunsetViewsTests['test1']['direction'] as string
);
